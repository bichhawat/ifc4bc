...
// IFC4BC
bool JSObject::getOwnPropertySlotByIndexIFC(JSCell* cell, ExecState* exec, unsigned propertyName, PropertySlot& slot, JSLabel* context)
{
    JSObject* thisObject = jsCast<JSObject*>(cell);
    return thisObject->methodTable()->getOwnPropertySlotIFC(thisObject, exec, Identifier::from(exec, propertyName), slot, context);
}
...
// ECMA 8.6.2.2
void JSObject::putIFC(JSCell* cell, ExecState* exec, PropertyName propertyName, JSValue value, PutPropertySlot& slot, JSLabel* contextLabel, bool *abortRun, bool* isStructChanged)
{
    JSObject* thisObject = jsCast<JSObject*>(cell);
    ASSERT(value);
    ASSERT(!Heap::heap(value) || Heap::heap(value) == Heap::heap(thisObject));
    ...
    //IFC4BC
    JSLabel increasingContextLabel = JSLabel();
    if (contextLabel) increasingContextLabel = *contextLabel;
    if (abortRun)
        *abortRun = false;
    ...
    if (propertyName != exec->propertyNames().underscoreProto) {
        for (JSObject* obj = thisObject; !obj->structure()->hasReadOnlyOrGetterSetterPropertiesExcludingProto(); obj = asObject(prototype)) {
            ...
            // if (contextLabel)
            increasingContextLabel = increasingContextLabel.Join(obj->getObjectLabel()).Join(obj->structure()->getProtoLabel());
            //------
            prototype = obj->prototype();
            if (prototype.isNull()) {
                //IFC4BC
                ...
                if (contextLabel){
                    if (!increasingContextLabel.NSU(thisObject->getObjectLabel())){
                        // printf("Traversed the prototype chain and found nothing\n");
                        if (abortRun)
                            *abortRun = true;
                ...
                //------
                if (!thisObject->putDirectInternalIFC<PutModePut>(globalData, propertyName, value, 0, slot, getCallableObject(value), abortRun, isStructChanged) && slot.isStrictMode())
                    throwTypeError(exec, StrictModeReadonlyPropertyWriteError);
                // IFC4BC - Reset the structure label of the object
                if (contextLabel){
                    // *contextLabel = increasingContextLabel;
                }
                return;
    ...
    // IFC4BC - Reset the context label as it traverses the whole chain again
    if (contextLabel)
        increasingContextLabel = *contextLabel;
        ...
        if (offset != invalidOffset) {
            ...
            if (gs.isGetterSetter()) {
                ...
                if (!setterFunc) {
                    if (slot.isStrictMode())
                        throwError(exec, createTypeError(exec, "setting a property that has only a getter"));
                    if (contextLabel)   *contextLabel = increasingContextLabel;
                    return;
                }
                ...
                //IFC4BC - Check if the nsu fails
                if (contextLabel){
                    if (!increasingContextLabel.NSU(obj->getObjectLabel())){
                        if (abortRun)
                            *abortRun = true;
                        *contextLabel = increasingContextLabel;
                        // return;
...
// IFC4BC
void JSObject::putByIndexIFC(JSCell* cell, ExecState* exec, unsigned propertyName, JSValue value, bool shouldThrow, JSLabel* context, bool* abort, bool* iSC)
{
    PutPropertySlot slot(shouldThrow);
    JSObject* thisObject = jsCast<JSObject*>(cell);
    thisObject->methodTable()->putIFC(thisObject, exec, Identifier::from(exec, propertyName), value, slot, context, abort, iSC);
}
...
// IFC4BC
bool JSObject::hasPropertyIFC(ExecState* exec, PropertyName propertyName, JSLabel *context) const
{
    PropertySlot slot;
    return const_cast<JSObject*>(this)->getPropertySlotIFC(exec, propertyName, slot, context);
}

bool JSObject::hasPropertyIFC(ExecState* exec, unsigned propertyName, JSLabel *context) const
{
    PropertySlot slot;
    return const_cast<JSObject*>(this)->getPropertySlotIFC(exec, propertyName, slot, context);
}
