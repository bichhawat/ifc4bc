...
// IFC4BC - IFC version of the function puttoprimitive
void JSValue::putToPrimitiveIFC(ExecState* exec, PropertyName propertyName, JSValue value, PutPropertySlot& slot, JSLabel* contextLabel, bool *abortRun)
{
    //IFC4BC
    ...
    for (; ; obj = asObject(prototype)) {
        ...
        if (offset != invalidOffset) {
            ...
            if (gs.isGetterSetter()) {
                ...
                //obj->setLabel(obj->joinLabel(increasingContextLabel));
                return;
            }
...
JSString* JSValue::toStringSlowCase(ExecState* exec) const
{
    ...
    ASSERT(!isString());
    if (isInt32()) {
        ret = jsString(&globalData, globalData.numericStrings.add(asInt32()));
        ret->setObjectLabel(this->getValueLabel());
        return ret;
    }
    if (isDouble()) {
        ret = jsString(&globalData, globalData.numericStrings.add(asDouble()));
        ret->setObjectLabel(this->getValueLabel());
        return ret;
    }
    if (isTrue()) {
        ret = globalData.smallStrings.trueString(&globalData);
        ret->setObjectLabel(this->getValueLabel());
        return ret;
    }
    if (isFalse()) {
        ret = globalData.smallStrings.falseString(&globalData);
        ret->setObjectLabel(this->getValueLabel());
        return ret;
    }
    if (isNull()) {
        ret = globalData.smallStrings.nullString(&globalData);
        ret->setObjectLabel(this->getValueLabel());
        return ret;
    }
    if (isUndefined()) {
        ret = globalData.smallStrings.undefinedString(&globalData);
        ret->setObjectLabel(this->getValueLabel());
        return ret;
    }
    ...
    value.setValueLabel(this->getValueLabel());
...
// IFC4BC - Functionality for labels of the value
JSLabel JSValue::getValueLabel() const
{
//    if (isCell()) {
//#if USE(JSVALUE32_64)
//        ASSERT(tag() != EmptyValueTag);
//        ASSERT(tag() != DeletedValueTag); 
//#else
//        if (u.asInt64 == ValueEmpty || u.asInt64 == ValueDeleted)
//        {
//            return JSLabel();
//        }
//#endif
...
void JSValue::setValueLabel(JSLabel l)
{
//    if (isCell()) {
//#if USE(JSVALUE32_64)
//        ASSERT(tag() != EmptyValueTag);
...
JSLabel JSValue::joinValueLabel(JSLabel l)
{
//    if (isCell()) {
//#if USE(JSVALUE32_64)
//        ASSERT(tag() != EmptyValueTag);
...
void  JSValue::updateValueLabel(JSLabel l)
{
    setValueLabel(joinValueLabel(l));
}

void JSValue::updateValueLabel(JSValue v)
{
    setValueLabel(joinValueLabel(v.getValueLabel()));
}
// IFC4BC -----------------------------
