...
inline void SparseArrayValueMap::put(ExecState* exec, JSArray* array, unsigned i, JSValue value, bool shouldThrow)
{
    ...
    args.append(value);
    array->setObjectLabel(array->joinObjectLabel(value.getValueLabel()));
...
// IFC4BC
void JSArray::putIFC(JSCell* cell, ExecState* exec, PropertyName propertyName, JSValue value, PutPropertySlot& slot, JSLabel* context, bool* abort, bool* isStructChanged)
{
    JSArray* thisObject = jsCast<JSArray*>(cell);
    unsigned i = propertyName.asIndex();
    if (i != PropertyName::NotAnIndex) {
    ...
    JSObject::putIFC(thisObject, exec, propertyName, value, slot, context, abort, isStructChanged);
}
...
// IFC4BC - IFC method for putByIndex
void JSArray::putByIndexIFC(JSCell* cell, ExecState* exec, unsigned i, JSValue value, bool shouldThrow, JSLabel* context, bool* abort, bool* iSC)
{
    JSArray* thisObject = jsCast<JSArray*>(cell);
    thisObject->checkConsistency();
    ...
    if (UNLIKELY(i > MAX_ARRAY_INDEX)) {
        PutPropertySlot slot(shouldThrow);
        thisObject->methodTable()->putIFC(thisObject, exec, Identifier::from(exec, i), value, slot, context, abort, iSC);
        return;
    }
...
void JSArray::putByIndexBeyondVectorLength(ExecState* exec, unsigned i, JSValue value, bool shouldThrow)
{
    ...
    if (LIKELY(!map)) {
        ...
        if (LIKELY((isDenseEnoughForVector(i, storage->m_numValuesInVector)) && increaseVectorLength(globalData, i + 1))) {
            ...
            ++storage->m_numValuesInVector;
            this->setObjectLabel(this->joinObjectLabel(value.getValueLabel()));
            return;
        }
