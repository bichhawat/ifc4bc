...
// IFC4BC
void JSFunction::putIFC(JSCell* cell, ExecState* exec, PropertyName propertyName, JSValue value, PutPropertySlot& slot, JSLabel* context, bool* abort, bool* isStructChanged)
{
    JSFunction* thisObject = jsCast<JSFunction*>(cell);
    if (thisObject->isHostFunction()) {
        Base::putIFC(thisObject, exec, propertyName, value, slot, context, abort, isStructChanged);
        return;
    }
    if (propertyName == exec->propertyNames().prototype) {
        // Make sure prototype has been reified, such that it can only be overwritten
        ...
        // Don't allow this to be cached, since a [[Put]] must clear m_cachedInheritorID.
        PutPropertySlot dontCache;
        Base::putIFC(thisObject, exec, propertyName, value, dontCache, context, abort, isStructChanged);
        return;
    }
    if (thisObject->jsExecutable()->isStrictMode() && (propertyName == exec->propertyNames().arguments || propertyName == exec->propertyNames().caller)) {
        // This will trigger the property to be reified, if this is not already the case!
