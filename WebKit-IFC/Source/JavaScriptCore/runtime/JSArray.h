...
namespace JSC {
    ...
    public:
        ...
        inline void initializeIndex(JSGlobalData& globalData, unsigned i, JSValue v)
        {
            ...
            // IFC4BC - Set the value's label to the current pc
            v.setValueLabel(v.joinValueLabel(JSLabel(JSLabel::pcGlobalLabel)));
    ...
    protected:
        ...
        // IFC4BC
        static void putIFC(JSCell*, ExecState*, PropertyName, JSValue, PutPropertySlot&, JSLabel* = NULL, bool* = NULL, bool* = NULL);
...
#define MAX_STORAGE_VECTOR_INDEX (MAX_STORAGE_VECTOR_LENGTH - 1)
    ...
    inline JSArray* constructArray(ExecState* exec, Structure* arrayStructure, const ArgList& values)
    {
        ...
        JSLabel arrayLabel = JSLabel(JSLabel::pcGlobalLabel);
        ...
        for (unsigned i = 0; i < length; ++i) {
            ...
            arrayLabel = arrayLabel.Join(values.at(i).getValueLabel());
        }
        array->completeInitialization(length);
        array->setObjectLabel(arrayLabel);
        array->structure()->setProtoLabel(arrayLabel);
        return array;
    }
    ...
    inline JSArray* constructArray(ExecState* exec, Structure* arrayStructure, const JSValue* values, unsigned length)
    {
        ...
        for (unsigned i = 0; i < length; ++i) {
            ...
            arrayLabel = arrayLabel.Join(values[i].getValueLabel());
        }
        array->completeInitialization(length);
        array->setObjectLabel(arrayLabel);
        array->structure()->setProtoLabel(arrayLabel);
