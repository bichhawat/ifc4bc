...
static JSValue JSC_HOST_CALL stringFromCharCode(ExecState*);
...
static JSValue JSC_HOST_CALL constructWithStringConstructor(ExecState* exec)
{
    JSGlobalObject* globalObject = asInternalFunction(exec->callee())->globalObject();
    if (!exec->argumentCount())
        return (StringObject::create(exec, globalObject->stringObjectStructure()));
    
    return (StringObject::create(exec, globalObject->stringObjectStructure(), exec->argument(0).toString(exec)));
}
...
static JSValue JSC_HOST_CALL callStringConstructor(ExecState* exec)
{
    ...
    return (exec->argument(0).toString(exec));
}
