...
static JSValue JSC_HOST_CALL constructWithDateConstructor(ExecState* exec)
{
    ArgList args(exec);
    return (constructDate(exec, asInternalFunction(exec->callee())->globalObject(), args));
}
...
{
    return (jsNumber(parseDate(exec, exec->argument(0).toString(exec)->value(exec))));
}
...
static JSValue JSC_HOST_CALL dateUTC(ExecState* exec) 
{
    ...
    return (jsNumber(timeClip(gregorianDateTimeToMS(exec, t, ms, true))));
}
