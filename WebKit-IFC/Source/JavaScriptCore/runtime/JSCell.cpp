...
// IFC4BC
bool JSCell::getOwnPropertySlotIFC(JSCell* cell, ExecState* exec, PropertyName identifier, PropertySlot& slot, JSLabel* context)
{
    // This is not a general purpose implementation of getOwnPropertySlot.
    // It should only be called by JSValue::get.
    // It calls getPropertySlot, not getOwnPropertySlot.
    JSObject* object = cell->toObject(exec, exec->lexicalGlobalObject());
    slot.setBase(object);
    if (!object->getPropertySlotIFC(exec, identifier, slot, context))
        slot.setUndefined();
    return true;
}

bool JSCell::getOwnPropertySlotByIndexIFC(JSCell* cell, ExecState* exec, unsigned identifier, PropertySlot& slot, JSLabel* context)
{
    // This is not a general purpose implementation of getOwnPropertySlot.
    // It should only be called by JSValue::get.
    // It calls getPropertySlot, not getOwnPropertySlot.
...
// IFC4BC
void JSCell::putIFC(JSCell* cell, ExecState* exec, PropertyName identifier, JSValue value, PutPropertySlot& slot, JSLabel* contextLabel, bool *abortRun, bool *isStructChanged)
{
    if (cell->isString()) {
        JSValue(cell).putToPrimitive(exec, identifier, value, slot);
        return;
    ...
    thisObject->methodTable()->putIFC(thisObject, exec, identifier, value, slot,contextLabel, abortRun, isStructChanged);
...
// IFC4BC
void JSCell::putByIndexIFC(JSCell* cell, ExecState* exec, unsigned identifier, JSValue value, bool shouldThrow, JSLabel* context, bool* abort, bool* iSC)
{
    if (cell->isString()) {
        PutPropertySlot slot(shouldThrow);
        JSValue(cell).putToPrimitiveIFC(exec, Identifier::from(exec, identifier), value, slot, context, abort);
        return;
    }
    JSObject* thisObject = cell->toObject(exec, exec->lexicalGlobalObject());
    thisObject->methodTable()->putByIndexIFC(thisObject, exec, identifier, value, shouldThrow, context, abort, iSC);
}
