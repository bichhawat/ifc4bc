...
// IFC4BC - Methods for IFC
void Arguments::putByIndexIFC(JSCell* cell, ExecState* exec, unsigned i, JSValue value, bool shouldThrow, JSLabel* context, bool* abort, bool* iSC)
{
    Arguments* thisObject = jsCast<Arguments*>(cell);
    if (i < static_cast<unsigned>(thisObject->d->numArguments) && (!thisObject->d->deletedArguments || !thisObject->d->deletedArguments[i])) {
        thisObject->argument(i).set(exec->globalData(), thisObject, value);
    ...
    PutPropertySlot slot(shouldThrow);
    JSObject::putIFC(thisObject, exec, Identifier(exec, UString::number(i)), value, slot, context, abort, iSC);
}

void Arguments::putIFC(JSCell* cell, ExecState* exec, PropertyName propertyName, JSValue value, PutPropertySlot& slot, JSLabel* context, bool* abort, bool* iSC)
{
    Arguments* thisObject = jsCast<Arguments*>(cell);
    unsigned i = propertyName.asIndex();
    if (i < thisObject->d->numArguments && (!thisObject->d->deletedArguments || !thisObject->d->deletedArguments[i])) {
    ...
    if (propertyName == exec->propertyNames().length && !thisObject->d->overrodeLength) {
        thisObject->d->overrodeLength = true;
        thisObject->putDirectIFC(exec->globalData(), propertyName, value, DontEnum, abort, iSC);
        return;
    }
    ...
    JSObject::putIFC(thisObject, exec, propertyName, value, slot, context, abort, iSC);
}
