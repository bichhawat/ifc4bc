...
JSValue JSC_HOST_CALL errorProtoFuncToString(ExecState* exec)
{
    ...
    if (!nameString.length())
        return (message.isString() ? message : jsString(exec, messageString));

    // 9. If msg is the empty String, return name.
    if (!messageString.length())
        return (name.isString() ? name : jsNontrivialString(exec, nameString));

    // 10. Return the result of concatenating name, ":", a single space character, and msg.
    return (jsMakeNontrivialString(exec, nameString, ": ", messageString));
}
