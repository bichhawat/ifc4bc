...
JSValue JSC_HOST_CALL numberProtoFuncToExponential(ExecState* exec)
{
    ...
    if (!isfinite(x))
        return (jsString(exec, UString::number(x)));
    ...
    return (jsString(exec, UString(builder.Finalize())));
}
...
JSValue JSC_HOST_CALL numberProtoFuncToFixed(ExecState* exec)
{
    ...
    return (jsString(exec, UString(numberToFixedWidthString(x, decimalPlaces, buffer))));
}
...
JSValue JSC_HOST_CALL numberProtoFuncToPrecision(ExecState* exec)
{
    ...
    NumberToStringBuffer buffer;
    return (jsString(exec, UString(numberToFixedPrecisionString(x, significantFigures, buffer))));
}
...
static inline JSValue integerValueToString(ExecState* exec, int32_t radix, int32_t value)
{
    ...
    if (static_cast<unsigned>(value) < static_cast<unsigned>(radix)) {
        ...
        return (globalData->smallStrings.singleCharacterString(globalData, radixDigits[value]));
    }

    if (radix == 10) {
        JSGlobalData* globalData = &exec->globalData();
        return (jsString(globalData, globalData->numericStrings.add(value)));
    }

    return (jsString(exec, toUStringWithRadix(value, radix)));
...
JSValue JSC_HOST_CALL numberProtoFuncToString(ExecState* exec)
{
    ...
    if (radix == 10) {
        JSGlobalData* globalData = &exec->globalData();
        return (jsString(globalData, globalData->numericStrings.add(doubleValue)));
    }

    if (!isfinite(doubleValue))
        return (jsString(exec, UString::number(doubleValue)));

    RadixBuffer s;
    return (jsString(exec, toStringWithRadix(s, doubleValue, radix)));
}
...
JSValue JSC_HOST_CALL numberProtoFuncToLocaleString(ExecState* exec)
{
    ...
    return (jsNumber(x).toString(exec));
}
