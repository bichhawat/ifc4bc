...
JSValue JSC_HOST_CALL globalFuncEval(ExecState* exec)
{
    ...
    return (exec->interpreter()->execute(eval, exec, thisObject, jsCast<JSGlobalObject*>(unwrappedObject)->globalScopeChain()));
}
...
JSValue JSC_HOST_CALL globalFuncParseInt(ExecState* exec)
{
    ...
    if (value.isNumber()) {
        ...
        if (((n < intMaxPlusOne && n >= tenToTheMinus6) || !n) && radixValue.isUndefinedOrNull())
            return (jsNumber(static_cast<int32_t>(n)));
    }
    ...
    return (jsNumber(parseInt(s, radixValue.toInt32(exec))));
}

JSValue JSC_HOST_CALL globalFuncParseFloat(ExecState* exec)
{
    return (jsNumber(parseFloat(exec->argument(0).toString(exec)->value(exec))));
}
...
JSValue JSC_HOST_CALL globalFuncUnescape(ExecState* exec)
{
    ...
    return (jsString(exec, builder.toUString()));
}

JSValue JSC_HOST_CALL globalFuncThrowTypeError(ExecState* exec)
{
    return throwVMTypeError(exec);
}

JSValue JSC_HOST_CALL globalFuncProtoGetter(ExecState* exec)
{
    if (!exec->thisValue().isObject())
        return (exec->thisValue().synthesizePrototype(exec));

    JSObject* thisObject = asObject(exec->thisValue());
    if (!thisObject->allowsAccessFrom(exec->trueCallerFrame()))
        return (jsUndefined());

    return (thisObject->prototype());
}

JSValue JSC_HOST_CALL globalFuncProtoSetter(ExecState* exec)
{
    JSValue value = exec->argument(0);
