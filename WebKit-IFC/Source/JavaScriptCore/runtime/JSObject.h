...
namespace JSC {
    ...
    public:
        ...
        // IFC4BC
        JSValue getIFC(ExecState*, PropertyName, JSLabel*) const;
        JSValue getIFC(ExecState*, unsigned propertyName, JSLabel*) const;

        bool getPropertySlot(ExecState*, PropertyName, PropertySlot&);
        bool getPropertySlot(ExecState*, unsigned propertyName, PropertySlot&);
        // IFC4BC
        bool getPropertySlotIFC(ExecState*, PropertyName, PropertySlot&, JSLabel*);
        bool getPropertySlotIFC(ExecState*, unsigned propertyName, PropertySlot&, JSLabel*);
        ...
        static bool getOwnPropertySlotIFC(JSCell*, ExecState*, PropertyName, PropertySlot&, JSLabel*);
        JS_EXPORT_PRIVATE static bool getOwnPropertySlotByIndexIFC(JSCell*, ExecState*, unsigned propertyName, PropertySlot&, JSLabel*);
        
        JS_EXPORT_PRIVATE static void putIFC(JSCell*, ExecState*, PropertyName, JSValue, PutPropertySlot&, JSLabel* contextLabel=NULL, bool *abortRun = NULL, bool *isStructChanged = NULL);
        JS_EXPORT_PRIVATE static void putByIndexIFC(JSCell*, ExecState*, unsigned propertyName, JSValue, bool shouldThrow, JSLabel*, bool*, bool*);
        ...
        //IFC4BC
        void putDirectIFC(JSGlobalData&, PropertyName, JSValue, unsigned attributes = 0, bool* = NULL, bool* = NULL);
        void putDirectIFC(JSGlobalData&, PropertyName, JSValue, PutPropertySlot&, bool* = NULL, bool* isStructChanged=NULL);
        //--------
        ...
        // IFC4BC
        JS_EXPORT_PRIVATE bool hasPropertyIFC(ExecState*, PropertyName, JSLabel*) const;
        JS_EXPORT_PRIVATE bool hasPropertyIFC(ExecState*, unsigned propertyName, JSLabel*) const;
    ...
    private:
        ...
        //IFC4BC
        template<PutMode>
        bool putDirectInternalIFC(JSGlobalData&, PropertyName, JSValue, unsigned attr, PutPropertySlot&, JSCell*, bool* isStructChanged=NULL, bool* = NULL);
        //-------
...
// IFC4BC
ALWAYS_INLINE bool JSObject::getOwnPropertySlotIFC(JSCell* cell, ExecState* exec, PropertyName propertyName, PropertySlot& slot, JSLabel*)
{
    return jsCast<JSObject*>(cell)->inlineGetOwnPropertySlot(exec, propertyName, slot);
}

ALWAYS_INLINE bool JSCell::fastGetOwnPropertySlotIFC(ExecState* exec, PropertyName propertyName, PropertySlot& slot, JSLabel* context)
{
    if (!structure()->typeInfo().overridesGetOwnPropertySlot())
        return asObject(this)->inlineGetOwnPropertySlot(exec, propertyName, slot);
    return methodTable()->getOwnPropertySlotIFC(this, exec, propertyName, slot, context);
}
...
// IFC4BC
ALWAYS_INLINE bool JSObject::getPropertySlotIFC(ExecState* exec, PropertyName propertyName, PropertySlot& slot, JSLabel* contextLabel)
{
    // IFC4BC
    JSObject* object = this;
    JSLabel increasingContextLabel = *contextLabel;
    while (true) {
        // IFC4BC - Joining the structure label of the object
        increasingContextLabel = increasingContextLabel.Join(object->getObjectLabel());
        // This should have been the actual check - Commenting until the issue is found
        // if (object->fastGetOwnPropertySlotIFC(exec, propertyName, slot, &increasingContextLabel)) {
        if (object->fastGetOwnPropertySlot(exec, propertyName, slot)) {
            *contextLabel = increasingContextLabel;
            return true;
        }
        // IFC4BC - Joining the prototype chain label with the context
        increasingContextLabel = increasingContextLabel.Join(object->structure()->getProtoLabel());
        JSValue prototype = object->prototype();
        if (!prototype.isObject()) {
            *contextLabel = increasingContextLabel;
            return false;
...
ALWAYS_INLINE bool JSObject::getPropertySlotIFC(ExecState* exec, unsigned propertyName, PropertySlot& slot, JSLabel* contextLabel)
{
    JSObject* object = this;
    JSLabel increasingContextLabel = *contextLabel;
    while (true) {
        increasingContextLabel = increasingContextLabel.Join(object->getObjectLabel());
        // if (object->methodTable()->getOwnPropertySlotByIndexIFC(object, exec, propertyName, slot, &increasingContextLabel)) {
        if (object->methodTable()->getOwnPropertySlotByIndex(object, exec, propertyName, slot)) {
            *contextLabel = increasingContextLabel;
            return true;
        }
...
// IFC4BC - JSObject get methods for IFC, the context is returned directly. Use it there.
inline JSValue JSObject::getIFC(ExecState* exec, PropertyName propertyName, JSLabel* context) const
{
    PropertySlot slot(this);
    if (const_cast<JSObject*>(this)->getPropertySlotIFC(exec, propertyName, slot, context)) {
        JSValue ret = slot.getValue(exec, propertyName);
        // ret.setValueLabel(ret.joinValueLabel(*context));
        return ret;
    }
...
inline JSValue JSObject::getIFC(ExecState* exec, unsigned propertyName, JSLabel* context) const
{
    PropertySlot slot(this);
    if (const_cast<JSObject*>(this)->getPropertySlotIFC(exec, propertyName, slot, context)) {
        JSValue ret = slot.getValue(exec, propertyName);
...
// IFC4BC - Replicating the function with the parameter to check if the structure has changed for IFC
template<JSObject::PutMode mode>
inline bool JSObject::putDirectInternalIFC(JSGlobalData& globalData, PropertyName propertyName, JSValue value, unsigned attributes, PutPropertySlot& slot, JSCell* specificFunction, bool* abortRun, bool* isStructChanged)
{
    ASSERT(value);
    ASSERT(value.isGetterSetter() == !!(attributes & Accessor));
    ASSERT(!Heap::heap(value) || Heap::heap(value) == Heap::heap(this));
    
    if (isStructChanged) *isStructChanged=false;
    ...
    if (structure()->isDictionary()) {
        ...
        if (offset != invalidOffset) {
            ...
            // IFC4BC - This check is already done before coming here, but done here for direct cases
            
            if (!(JSLabel(JSLabel::pcGlobalLabel)).NSU(this->getObjectLabel())) {
                *abortRun = true;
                // return true;
            }
            JSLabel valLabel = value.getValueLabel();
            value.setValueLabel(valLabel.Join(JSLabel(JSLabel::pcGlobalLabel)));
        ...
        // JSLabel valLabel = value.getValueLabel();
        // valLabel.setStar(true);
        // value.setValueLabel(valLabel);
        putDirectOffset(globalData, offset, value);
        // See comment on setNewProperty call below.
        if (!specificFunction)
            slot.setNewProperty(this, offset);
        //IFC4BC
        if (isStructChanged)    *isStructChanged=true;
        //--------
        return true;
    }
    ...
    if (Structure* structure = Structure::addPropertyTransitionToExistingStructure(this->structure(), propertyName, attributes, specificFunction, offset)) {
        ...
        ASSERT(structure->isValidOffset(offset));
        setOutOfLineStorage(globalData, newStorage, structure);
        // Set that it is a userdefined property
        // JSLabel valLabel = value.getValueLabel();
        // valLabel.setStar(true);
        // value.setValueLabel(valLabel);
        putDirectOffset(globalData, offset, value);
    ...
    if (offset != invalidOffset) {
        ...
        // IFC4BC - If the existing property contains a label, then keep this, else make it zero.
...
// IFC4BC -- Creating a new function for the IFC
inline void JSObject::putDirectIFC(JSGlobalData& globalData, PropertyName propertyName, JSValue value, unsigned attributes, bool* abortRun, bool* isStructChanged)
{
    ASSERT(!value.isGetterSetter() && !(attributes & Accessor));
    PutPropertySlot slot;
    putDirectInternalIFC<PutModeDefineOwnProperty>(globalData, propertyName, value, attributes, slot, getCallableObject(value), abortRun, isStructChanged);
}

inline void JSObject::putDirectIFC(JSGlobalData& globalData, PropertyName propertyName, JSValue value, PutPropertySlot& slot, bool* abortRun, bool* isStructChanged)
{
    ASSERT(!value.isGetterSetter());
    putDirectInternalIFC<PutModeDefineOwnProperty>(globalData, propertyName, value, 0, slot, getCallableObject(value), abortRun, isStructChanged);
}
...
// IFC4BC
inline JSValue JSValue::getIFC(ExecState* exec, PropertyName propertyName, JSLabel* context) const
{
    PropertySlot slot(asValue());
    return getIFC(exec, propertyName, slot, context);
}

inline JSValue JSValue::getIFC(ExecState* exec, PropertyName propertyName, PropertySlot& slot, JSLabel* contextLabel) const
{
    //IFC4BC
    JSLabel increasingContextLabel = *contextLabel;
    //-------
    if (UNLIKELY(!isCell())) {
        JSObject* prototype = synthesizePrototype(exec);
        if (!prototype->getPropertySlotIFC(exec, propertyName, slot, &increasingContextLabel)){
            JSValue ret = jsUndefined();
    ...
    JSCell* cell = asCell();
    while (true) {
        increasingContextLabel = increasingContextLabel.Join(cell->getObjectLabel());
        if (cell->fastGetOwnPropertySlot(exec, propertyName, slot)){
            JSValue ret =  slot.getValue(exec, propertyName);
            //printf("gfdgfdgfd gdfgfdgfd jin %s\n", ret.toUString(exec).utf8().data());
            // ret.setValueLabel(ret.joinValueLabel(increasingContextLabel));
            *contextLabel = increasingContextLabel;
            return ret;
        }
        ...
        cell = asObject(prototype);
        increasingContextLabel = increasingContextLabel.Join(cell->structure()->getProtoLabel());
        //---------
    }
}
...
// IFC4BC
inline JSValue JSValue::getIFC(ExecState* exec, unsigned propertyName, JSLabel* context) const
{
    PropertySlot slot(asValue());
    return getIFC(exec, propertyName, slot, context);
}

inline JSValue JSValue::getIFC(ExecState* exec, unsigned propertyName, PropertySlot& slot, JSLabel* contextLabel) const
{
    //IFC4BC
    JSLabel increasingContextLabel = *contextLabel;
    //-------
...
inline void JSValue::putIFC(ExecState* exec, PropertyName propertyName, JSValue value, PutPropertySlot& slot, JSLabel* contextLabel, bool *abortRun, bool *isStructChanged)
{
    if (UNLIKELY(!isCell())) {
        putToPrimitiveIFC(exec, propertyName, value, slot, contextLabel, abortRun);
        return;
    }
    
    //IFC4BC
    asCell()->methodTable()->putIFC(asCell(), exec, propertyName, value, slot, contextLabel, abortRun, isStructChanged);
}
...
// IFC4BC
inline void JSValue::putByIndexIFC(ExecState* exec, unsigned propertyName, JSValue value, bool shouldThrow, JSLabel* context, bool* abort, bool* iSC)
{
    if (UNLIKELY(!isCell())) {
        PutPropertySlot slot(shouldThrow);
        putToPrimitiveIFC(exec, Identifier::from(exec, propertyName), value, slot, context, abort);
        return;
    }
    asCell()->methodTable()->putByIndexIFC(asCell(), exec, propertyName, value, shouldThrow, context, abort, iSC);
}
