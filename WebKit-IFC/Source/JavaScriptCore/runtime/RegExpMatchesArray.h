...
namespace JSC {
    ...
    private:
        ...
        // IFC4BC
        static void putIFC(JSCell* cell, ExecState* exec, PropertyName propertyName, JSValue v, PutPropertySlot& slot, JSLabel* context, bool* abort, bool* iSC)
        {
            RegExpMatchesArray* thisObject = jsCast<RegExpMatchesArray*>(cell);
            thisObject->reifyAllPropertiesIfNecessary(exec);
            JSArray::putIFC(thisObject, exec, propertyName, v, slot, context, abort, iSC);
        }
        
        static void putByIndexIFC(JSCell* cell, ExecState* exec, unsigned propertyName, JSValue v, bool shouldThrow, JSLabel* context, bool* abort, bool* iSC)
        {
            RegExpMatchesArray* thisObject = jsCast<RegExpMatchesArray*>(cell);
            thisObject->reifyAllPropertiesIfNecessary(exec);
            JSArray::putByIndexIFC(thisObject, exec, propertyName, v, shouldThrow, context, abort, iSC);
        }
