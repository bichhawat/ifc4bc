...
JSValue JSC_HOST_CALL regExpProtoFuncTest(ExecState* exec)
{
    ...
    return (jsBoolean(asRegExpObject(thisValue)->test(exec, exec->argument(0).toString(exec))));
}
...
JSValue JSC_HOST_CALL regExpProtoFuncExec(ExecState* exec)
{
    ...
    if (!thisValue.inherits(&RegExpObject::s_info))
        return throwVMTypeError(exec);
    return (asRegExpObject(thisValue)->exec(exec, exec->argument(0).toString(exec)));
}
...
JSValue JSC_HOST_CALL regExpProtoFuncToString(ExecState* exec)
{
    ...
    return (jsMakeNontrivialString(exec, "/", source, postfix));
}
