...
static JSValue JSC_HOST_CALL functionProtoFuncBind(ExecState*);
    ...
    if (!array.isUndefinedOrNull()) {
        ...
        if (asObject(array)->classInfo() == &Arguments::s_info) {
            if (asArguments(array)->length(exec) > Arguments::MaxArguments)
                return (throwStackOverflowError(exec));
...
JSValue JSC_HOST_CALL functionProtoFuncBind(ExecState* exec)
{
    ...
    return (JSBoundFunction::create(exec, globalObject, targetObject, exec->argument(0), boundArgs, length, name->value(exec)));
}
