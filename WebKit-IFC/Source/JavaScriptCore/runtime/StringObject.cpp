...
// IFC4BC
void StringObject::putIFC(JSCell* cell, ExecState* exec, PropertyName propertyName, JSValue value, PutPropertySlot& slot, JSLabel* context, bool* abort, bool* iSC)
{
    if (propertyName == exec->propertyNames().length) {
        if (slot.isStrictMode())
            throwTypeError(exec, StrictModeReadonlyPropertyWriteError);
    ...
    JSObject::putIFC(cell, exec, propertyName, value, slot, context, abort, iSC);
}
