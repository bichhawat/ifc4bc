...
namespace JSC {
    ...
    ALWAYS_INLINE JSValue resolveBase(CallFrame* callFrame, Identifier& property, ScopeChainNode* scopeChain, bool isStrictPut, JSLabel *contextLabel)
    {
        ...
        PropertySlot slot;
        JSObject* base;
        if (contextLabel) { // IFC4BC
            while (true) {
                base = iter->get();
                increasingContextLabel = increasingContextLabel.Join(base->getObjectLabel());
                if (next == end) {
                    if (isStrictPut && !base->getPropertySlotIFC(callFrame, property, slot, &increasingContextLabel))
                    {
                        JSValue ret = JSValue();
                        if (contextLabel) {
                            ret.setValueLabel(increasingContextLabel);
                            *contextLabel = increasingContextLabel;
                        }
                        return ret;
                    }
                    JSValue retBase;
                    retBase = JSValue(base);
                    if (contextLabel) {
                        retBase.setValueLabel(increasingContextLabel);
                        *contextLabel = increasingContextLabel;
                    }
                    return retBase;
                }
                if (base->getPropertySlotIFC(callFrame, property, slot, &increasingContextLabel)){
                    //IFC4BC
                    JSValue retBase;
                    retBase = JSValue(base);
                    if(contextLabel) {
