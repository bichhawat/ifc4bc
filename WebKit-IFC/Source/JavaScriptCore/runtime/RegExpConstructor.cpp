...
static JSValue JSC_HOST_CALL constructWithRegExpConstructor(ExecState* exec)
{
    ArgList args(exec);
    return (constructRegExp(exec, asInternalFunction(exec->callee())->globalObject(), args, true));
}
...
{
    ArgList args(exec);
    return (constructRegExp(exec, asInternalFunction(exec->callee())->globalObject(), args));
}
