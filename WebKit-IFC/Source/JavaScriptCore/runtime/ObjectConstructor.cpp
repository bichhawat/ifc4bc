...
static JSValue JSC_HOST_CALL objectConstructorSeal(ExecState*);
static JSValue JSC_HOST_CALL objectConstructorFreeze(ExecState*);
static JSValue JSC_HOST_CALL objectConstructorPreventExtensions(ExecState*);
static JSValue JSC_HOST_CALL objectConstructorIsSealed(ExecState*);
static JSValue JSC_HOST_CALL objectConstructorIsFrozen(ExecState*);
static JSValue JSC_HOST_CALL objectConstructorIsExtensible(ExecState*);
...
static JSValue JSC_HOST_CALL constructWithObjectConstructor(ExecState* exec)
{
    ArgList args(exec);
    return (constructObject(exec, asInternalFunction(exec->callee())->globalObject(), args));
}
...
JSValue JSC_HOST_CALL objectConstructorDefineProperties(ExecState* exec)
{
    if (!exec->argument(0).isObject())
        return throwVMError(exec, createTypeError(exec, "Properties can only be defined on Objects."));
    return (defineProperties(exec, asObject(exec->argument(0)), exec->argument(1).toObject(exec)));
}
...
JSValue JSC_HOST_CALL objectConstructorSeal(ExecState* exec)
{
...
JSValue JSC_HOST_CALL objectConstructorFreeze(ExecState* exec)
{
    // 1. If Type(O) is not Object throw a TypeError exception.
    JSValue obj = exec->argument(0);
    if (!obj.isObject())
...
JSValue JSC_HOST_CALL objectConstructorPreventExtensions(ExecState* exec)
{
    JSValue obj = exec->argument(0);
    if (!obj.isObject())
...
JSValue JSC_HOST_CALL objectConstructorIsSealed(ExecState* exec)
{
    // 1. If Type(O) is not Object throw a TypeError exception.
    JSValue obj = exec->argument(0);
    if (!obj.isObject())
    ...
    if (isJSFinalObject(object))
        return (jsBoolean(object->isSealed(exec->globalData())));
    ...
    return (jsBoolean(!object->isExtensible()));
}

JSValue JSC_HOST_CALL objectConstructorIsFrozen(ExecState* exec)
{
    // 1. If Type(O) is not Object throw a TypeError exception.
    JSValue obj = exec->argument(0);
    if (!obj.isObject())
    ...
    if (isJSFinalObject(object))
        return (jsBoolean(object->isFrozen(exec->globalData())));
...
JSValue JSC_HOST_CALL objectConstructorIsExtensible(ExecState* exec)
{
    JSValue obj = exec->argument(0);
    if (!obj.isObject())
        return throwVMError(exec, createTypeError(exec, "Object.isExtensible can only be called on Objects."));
    return (jsBoolean(asObject(obj)->isExtensible()));
}
