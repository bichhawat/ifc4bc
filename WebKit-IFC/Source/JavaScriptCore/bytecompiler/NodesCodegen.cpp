...
RegisterID* TryNode::emitBytecode(BytecodeGenerator& generator, RegisterID* dst)
{
    ...
    if (m_finallyBlock) {
        ...
        // Abhi -- Changes to finally block code generated for getting immediate post-dominator
        RefPtr<Label> afterCatch = generator.newLabel();
        RefPtr<Label> afterThrow = generator.newLabel();
        RefPtr<RegisterID> tempExceptionRegister = generator.newTemporary();
        
        generator.emitMoveEmpty(tempExceptionRegister.get());
        generator.emitJump(afterCatch.get());
        RefPtr<Label> here = generator.emitLabel(generator.newLabel().get());
        tempExceptionRegister = generator.emitCatch(tempExceptionRegister.get(), tryStartLabel.get(), here.get());
        generator.emitLabel(afterCatch.get());
        generator.emitNode(dst, m_finallyBlock);
        generator.emitJumpIfEmpty(tempExceptionRegister.get(),afterThrow.get());
        generator.emitThrow(tempExceptionRegister.get());
        generator.emitLabel(afterThrow.get());
        // Abhi ------------------------------------------------------------------------------- */
    }
