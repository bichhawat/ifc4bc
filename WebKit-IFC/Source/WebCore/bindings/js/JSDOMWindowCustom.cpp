...
// IFC4BC
bool JSDOMWindow::getOwnPropertySlotIFC(JSCell* cell, ExecState* exec, PropertyName propertyName, PropertySlot& slot, JSLabel* context)
{
    JSDOMWindow* thisObject = jsCast<JSDOMWindow*>(cell);
    // When accessing a Window cross-domain, functions are always the native built-in ones, and they
    // are not affected by properties changed on the Window or anything in its prototype chain.
    ...
    // Look for overrides before looking at any of our own properties, but ignore overrides completely
    // if this is cross-domain access.
    if (allowsAccess && JSGlobalObject::getOwnPropertySlotIFC(thisObject, exec, propertyName, slot, context))
        return true;
    ...
    JSValue proto = thisObject->prototype();
    if (proto.isObject()) {
        if (asObject(proto)->getPropertySlotIFC(exec, propertyName, slot, context)) {
            if (!allowsAccess) {
                thisObject->printErrorMessage(errorMessage);
                slot.setUndefined();
            }
    ...
    return Base::getOwnPropertySlotIFC(thisObject, exec, propertyName, slot, context);
}
...
// IFC4BC
void JSDOMWindow::putIFC(JSCell* cell, ExecState* exec, PropertyName propertyName, JSValue value, PutPropertySlot& slot, JSLabel* context, bool* abort, bool* iSC)
{
    JSDOMWindow* thisObject = jsCast<JSDOMWindow*>(cell);
    if (!thisObject->impl()->frame())
        return;
    ...
    if (thisObject->JSGlobalObject::hasOwnPropertyForWrite(exec, propertyName)) {
        if (thisObject->allowsAccessFrom(exec))
            JSGlobalObject::putIFC(thisObject, exec, propertyName, value, slot, context, abort, iSC);
        return;
    }
    ...
    if (thisObject->allowsAccessFrom(exec))
        Base::putIFC(thisObject, exec, propertyName, value, slot, context, abort, iSC);
}
...
static JSValue handlePostMessage(DOMWindow* impl, ExecState* exec)
{
    ...
    // IFC4BC - Check here for postMessage sending data out
    JSC::JSLabel msgLabel = JSC::JSLabel::argLabel[1]; // 1st argument is the message
    JSC::JSLabel srcLabel = JSC::JSLabel(URLMap::urlmap().getLabel(targetOrigin.utf8().data()));
    if (!msgLabel.NSU(srcLabel))
    {
