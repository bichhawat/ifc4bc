...
public:
    ...
    // IFC4BC
    JSC::JSLabel getFCLabel() const { return fCLabel;}
    void setFCLabel(JSC::JSLabel l) { fCLabel = l;}
    void joinFCLabel(JSC::JSLabel l) { fCLabel = fCLabel.Join(l);}
    
    JSC::JSLabel getLCLabel() const { return lCLabel;}
    void setLCLabel(JSC::JSLabel l) { lCLabel = l;}
    void joinLCLabel(JSC::JSLabel l) { lCLabel = lCLabel.Join(l);}
...
protected:
    ...
    void setFirstChild(Node* child) {
        // IFC4BC - set the label of the child link
        if (JSC::JSLabel(JSC::JSLabel::pcGlobalLabel).NSU(getFCLabel()))
        {
            // IFC4BC - set the first child label of the parent
            setFCLabel(JSC::JSLabel(JSC::JSLabel::pcGlobalLabel).Join(JSC::JSLabel::argLabel[1]));
        }
        else
        {
            // printf("IFC check in setFirstChild failed\n");
    ...
    void setLastChild(Node* child) {
        if (JSC::JSLabel(JSC::JSLabel::pcGlobalLabel).NSU(getLCLabel()))
        {
            // IFC4BC - set the first child label of the parent
            setLCLabel(JSC::JSLabel(JSC::JSLabel::pcGlobalLabel).Join(JSC::JSLabel::argLabel[1]));
        }
        else
        {
            // printf("IFC check in setLastChild failed\n");
            JSC::JSLabel::ABORT_FLAG = true;
            // return;
        }
        m_lastChild = child;
...
inline ContainerNode::ContainerNode(Document* document, ConstructionType type)
    ...
    , m_lastChild(0)
{
    setFCLabel(JSC::JSLabel::pcGlobalLabel);
    setLCLabel(JSC::JSLabel::pcGlobalLabel);
}
...
inline JSC::JSLabel Node::getFCLabel() const
{
    if (!isContainerNode())
        return JSC::JSLabel();
    return toContainerNode(this)->getFCLabel();
}

inline JSC::JSLabel Node::getLCLabel() const
{
    if (!isContainerNode())
        return JSC::JSLabel();
    return toContainerNode(this)->getLCLabel();
}
    
inline JSC::JSLabel Node::getNextPointerLabel() const
{
    JSC::JSLabel label = JSC::JSLabel();
    if (firstChild())
    {
        label = label.Join(getFCLabel());
    }
    else if (nextSibling())
    {
        label = label.Join(getNextSiblingLabel());
    }
    else
    {
        for (Node* nodeUp = parentNode(); nodeUp; nodeUp = nodeUp->parentNode()) {
            if (nodeUp->nextSibling()) {
                label = label.Join(nodeUp->getNextSiblingLabel());
                break;
            }
        }
    }
