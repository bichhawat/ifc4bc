...
#include <JavaScriptCore/URLMap.h>
    ...
    , m_renderer(0)
{
    if (document) {
        document->guardRef();
        // IFC4BC -----------
        // JSC::URLMap::urlmap().put(baseURI().string().utf8().data());
        // const char* docURL = m_document->documentURI().utf8().data();
        // char *temp = strstr(docURL, ".policy");
        // JSC::JSLabel label;
        // if(temp) label = JSC::JSLabel(JSC::URLMap::urlmap().getLabel(docURL, 1));
        // label = JSC::JSLabel(JSC::URLMap::urlmap().getLabel(m_document->documentURI().utf8().data()));
         
        JSC::JSLabel pcLabel = JSC::JSLabel(JSC::JSLabel::pcGlobalLabel);
         
        // Joining pc Label to the newly created node's label
        // label = label.Join(pcLabel);
        // nodeLabel = label;
        // Assigning the docLabel, sibling and parent labels
        docLabel = pcLabel;
        nextSiblingLabel = pcLabel;
        prevSiblingLabel = pcLabel;
        setParentLabel(pcLabel);
        // printf("Node's label is %llu\n", nodeLabel.Val());
    }
