...
void ImageLoader::updateFromElement()
{
    ...
    if (!attr.isNull() && !stripLeadingAndTrailingHTMLSpaces(attr).isEmpty()) {
        // IFC4BC - Here, the image src loads a new image. We need to check the label of the data being sent here.
        // Also, check the PC if it is higher than the URL to which request being made.
        // printf("Request for an image: %s\n", sourceURI(attr).utf8().data());
        // TODO - USE NSU
        JSC::JSLabel urlLabel = JSC::JSLabel(JSC::URLMap::urlmap().getLabel(sourceURI(attr).utf8().data()));
